...
| `GET    /visitors?withVisits=true&name=X&email=Y&gender=Z&age_brackets=18,35,51,70&is_sms_contact_consent_granted=false&is_email_contact_consent_granted=false` | `Authorization: cb_admin_token` | | `{ result: [{ id, name, gender, yob, email, signup_date, qr_code_url, cb_logo_url, visits?: [{ activity, visit_date }] }] }` | `POST /visitors/all` <br/> `POST /visitors/filtered` <br/> `POST /visit/check` <br/> `POST /users/all` |
